.PHONY: check rust-check rust-static rust-test pgwire-fuzz-static agent-target-fence-test agent-wal-generation-test agent-reload-test catalog-test orch-catalog-test orch-slot-observer-test pgwire-postgres-test pooler-postgres-test planner-postgres-test proto-check go-check go-format-check go-generated-check docs-check docs-audit actions-check public-check images release-build
...
agent-target-fence-test:
	extensions/pgshard_fence/test.sh

agent-wal-generation-test:
	crates/pgshard-agent/tests/postgres18_generation.sh

agent-reload-test:
	@test -n "$(PGSHARD_TEST_POSTGRES_BIN)" || (echo "PGSHARD_TEST_POSTGRES_BIN is required" >&2; exit 1)
	cargo test --locked -p pgshard-agent --test postgres18_reload -- --ignored --test-threads=1
