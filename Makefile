...
go-check: go-format-check go-generated-check
	...
	cd operator && go vet ./...
	# Operator tests assert against pinned digests held outside the module, and
	# Go's test cache does not re-stat files above the module root, so a cached
	# pass would survive a Dockerfile-only drift. The flag trails the package
	# list because the release gate asserts this target runs `go test -race ./...`.
