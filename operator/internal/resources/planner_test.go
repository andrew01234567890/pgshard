...
func TestPlanIsDeterministicAndWiresGeneratedConfiguration(t *testing.T) {
	...
	if strings.Contains(databaseGenesis, "\\") || strings.Contains(databaseGenesis, "BEGIN TRANSACTION") || strings.Contains(databaseGenesis, "\nCOMMIT;") {
		t.Fatalf("database genesis is not pure caller-framed SQL:\n%s", databaseGenesis)
	}
	if !strings.HasPrefix(databaseGenesis, "-- Generated by pgshard-operator. Manual edits are overwritten.\nSET LOCAL search_path = pg_catalog;\n") {
		t.Fatalf("database genesis does not pin the caller-framed search path:\n%s", databaseGenesis)
	}
	...
	if strings.Contains(databasePreflight, "\\") || strings.Contains(databasePreflight, ":'") {
		t.Fatalf("database topology preflight is not pure caller-framed SQL:\n%s", databasePreflight)
	}
	if !strings.Contains(databasePreflight, "actual_databases AS MATERIALIZED") ||
		!strings.Contains(databasePreflight, "NOT COALESCE(pg_catalog.current_setting('pgshard.bootstrap_allow_empty_database_topology', true)::boolean, false)") ||
...
func TestShardInventoryHashMatchesCanonicalSource(t *testing.T) {
	t.Parallel()
	contents, err := os.ReadFile(filepath.Join("..", "..", "..", "crates", "pgshard-catalog", "inventory", "0001_shard_inventory.sql"))
	if err != nil {
		t.Fatal(err)
	}
	digest := sha256.Sum256(contents)
	if got := hex.EncodeToString(digest[:]); got != shardInventorySHA256 {
		t.Fatalf("shard inventory digest = %s, want %s", got, shardInventorySHA256)
	}
}
	...
	cluster := &pgshardv1alpha1.PgShardCluster{
		Spec: pgshardv1alpha1.PgShardClusterSpec{
			...
			Databases: []pgshardv1alpha1.DatabaseTemplate{
				{Name: "app"},
				{Name: "analytics", Cells: []int32{2, 0}},
				{Name: "billing", Shards: 2},
			},
		},
	}
	goldens := map[string]string{
		"genesis.golden.sql":   renderDatabaseGenesisSQL(cluster),
		"preflight.golden.sql": renderDatabaseTopologyPreflightSQL(cluster),
	}
	directory := filepath.Join("..", "..", "..", "crates", "pgshard-catalog", "testdata", "materialization")
	if *updateMaterializationGoldens {
		if err := os.MkdirAll(directory, 0o755); err != nil {
			t.Fatal(err)
		}
	}
	for name, rendered := range goldens {
		...
		if err != nil {
			t.Fatalf("read committed golden (regenerate with -update): %v", err)
		}
		if string(committed) != rendered {
			t.Fatalf("%s differs from the current renderer output; regenerate with -update", name)
		}
	}
}
...
func TestSingleMemberPlanCreatesPostgreSQL18Primaries(t *testing.T) {
	...
	for shard := int32(0); shard < cluster.Spec.Shards; shard++ {
		...
		if len(bootstrap.Env) != expectedEnvironmentLength || bootstrap.Env[0].Name != "PGSHARD_CLUSTER_UID" || bootstrap.Env[0].Value != string(cluster.UID) || bootstrap.Env[1].Name != "PGSHARD_SHARD_ID" || bootstrap.Env[1].Value != shardLabel(shard) ||
			...
			bootstrap.Env[8].Name != "PGSHARD_SHARD_INVENTORY" || bootstrap.Env[8].Value != shardInventoryPath ||
			bootstrap.Env[9].Name != "PGSHARD_SHARD_INVENTORY_SHA256" || bootstrap.Env[9].Value != shardInventorySHA256 ||
			bootstrap.Env[10].Name != "PGSHARD_POSTGRESQL_CONFIG_SHA256" || bootstrap.Env[10].Value != configurationHash ||
			bootstrap.Env[11].Name != "PGSHARD_NODE_UID" || bootstrap.Env[11].ValueFrom == nil || bootstrap.Env[11].ValueFrom.FieldRef == nil || bootstrap.Env[11].ValueFrom.FieldRef.FieldPath != "metadata.annotations['pgshard.io/postgresql-node-uid']" ||
			bootstrap.Env[12].Name != "PGSHARD_NODE_BOOT_ID" || bootstrap.Env[12].ValueFrom == nil || bootstrap.Env[12].ValueFrom.FieldRef == nil || bootstrap.Env[12].ValueFrom.FieldRef.FieldPath != "metadata.annotations['pgshard.io/postgresql-node-boot-id']" {
			t.Fatalf("PostgreSQL bootstrap identity = %#v", bootstrap.Env)
		}
		if shard == 0 && (bootstrap.Env[13].Name != "PGSHARD_CATALOG_CLIENT_SHA256" || bootstrap.Env[13].Value != cluster.Status.CatalogAccess.ClientSHA256 || bootstrap.Env[14].Name != "PGSHARD_CATALOG_SERVER_SHA256" || bootstrap.Env[14].Value != cluster.Status.CatalogAccess.ServerSHA256 || bootstrap.Env[15].Name != "PGSHARD_OPERATION_WRITER_SHA256" || bootstrap.Env[15].Value != cluster.Status.OperationWriterAccess.MaterialSHA256) {
...
func TestPostgreSQLBootstrapDockerRecoveryAndConflict(t *testing.T) {
	...
	bootstrapEnvironment := func(installCatalog bool, shardCount int) []string {
		return []string{
			...
			"PGSHARD_SHARD_INVENTORY=" + shardInventoryPath,
			"PGSHARD_SHARD_INVENTORY_SHA256=" + shardInventorySHA256,
...
func TestAgentQuarantineExplicitlyScopesOrchestratorIdentityObservation(t *testing.T) {
	...
	role := object[*rbacv1.Role](t, plan, "demo-orchestrator")
	candidateNames := make([]string, 0, len(cluster.Status.PostgreSQLCatalogCandidates))
	for _, checkpoint := range cluster.Status.PostgreSQLCatalogCandidates {
		candidateNames = append(candidateNames, checkpoint.ConfigMapName)
	}
...
func TestMultiMemberAgentPlanPublishesSourcesAndTCPClosedStandbys(t *testing.T) {
	...
	for _, members := range []int32{3, 5} {
		...
		t.Run(fmt.Sprintf("members=%d", members), func(t *testing.T) {
			...
			activationPolicy := object[*networkingv1.NetworkPolicy](t, plan, shardName(cluster.Name, 0)+"-catalog-activation-tls-ingress")
			wantActivationSelector := componentSelector(cluster, "postgresql")
			wantActivationSelector[ShardLabel] = "0000"
			wantActivationSelector[MemberLabel] = "0000"
			if !maps.Equal(activationPolicy.Spec.PodSelector.MatchLabels, wantActivationSelector) ||
				len(activationPolicy.Spec.Ingress) != 1 || len(activationPolicy.Spec.Ingress[0].From) != 1 ||
				activationPolicy.Spec.Ingress[0].From[0].PodSelector == nil ||
				!maps.Equal(activationPolicy.Spec.Ingress[0].From[0].PodSelector.MatchLabels, componentSelector(cluster, "orchestrator")) ||
				len(activationPolicy.Spec.Ingress[0].Ports) != 1 || activationPolicy.Spec.Ingress[0].Ports[0].Port == nil ||
				activationPolicy.Spec.Ingress[0].Ports[0].Port.IntVal != CatalogActivationTLSPort {
				t.Fatalf("catalog activation TLS NetworkPolicy = %#v", activationPolicy.Spec)
			}
			sources := 0
			standbys := 0
			for _, item := range plan {
				...
				case *appsv1.StatefulSet:
					...
					if member == "0000" {
						...
						if shard == 0 {
							...
							if tlsProjection == nil || tlsProjection.SecretName != cluster.Status.CatalogAccess.SecretName ||
								tlsProjection.DefaultMode == nil || *tlsProjection.DefaultMode != 0o440 ||
								!reflect.DeepEqual(secretItemKeys(tlsProjection.Items), []string{CatalogTLSCertificateKey, CatalogTLSPrivateKeyKey}) ||
								len(tlsProjection.Items) != 2 || tlsProjection.Items[0].Mode == nil || *tlsProjection.Items[0].Mode != 0o440 ||
...
func TestMultiMemberCatalogCandidateConfigurationsAreImmutableAndInert(t *testing.T) {
	...
	for _, members := range []int32{3, 5} {
		...
		t.Run(fmt.Sprintf("members=%d", members), func(t *testing.T) {
			...
			candidateNames := make(map[string]struct{}, members)
			for _, checkpoint := range cluster.Status.PostgreSQLCatalogCandidates {
				candidateNames[checkpoint.ConfigMapName] = struct{}{}
			}
			postgresqlConfiguration := object[*corev1.ConfigMap](t, plan, cluster.Status.PostgreSQLConfiguration.ConfigMapName)
			if PostgreSQLConfigurationDataSHA256(postgresqlConfiguration) != cluster.Status.PostgreSQLConfiguration.DataSHA256 {
				t.Fatal("planned PostgreSQL configuration does not match its exact checkpoint")
			}
			for member := int32(0); member < members; member++ {
				name := cluster.Status.PostgreSQLCatalogCandidates[member].ConfigMapName
				...
				wantedBundle, err := buildCatalogCandidateMaterializationBundle(
					cluster, postgresqlConfiguration, cluster.Status.PostgreSQLConfiguration,
					cluster.Status.CatalogAccess, cluster.Status.OperationWriterAccess,
				)
				if err != nil {
					t.Fatal(err)
				}
				if !reflect.DeepEqual(document.MaterializationBundle, wantedBundle) {
					t.Fatalf("catalog candidate %d materialization bundle = %#v, want %#v", member, document.MaterializationBundle, wantedBundle)
				}
				target := object[*appsv1.StatefulSet](t, plan, document.MaterializationBundle.TargetPodTemplate.StatefulSetName)
				if target.Spec.Template.Annotations[document.MaterializationBundle.TargetPodTemplate.ConfigurationAnnotation.Key] != document.MaterializationBundle.TargetPodTemplate.ConfigurationAnnotation.Value ||
					target.Spec.Template.Annotations[document.MaterializationBundle.TargetPodTemplate.ShardschemaMigrationAnnotation.Key] != document.MaterializationBundle.TargetPodTemplate.ShardschemaMigrationAnnotation.Value {
					t.Fatalf("catalog candidate %d target template annotations = %#v, bundle=%#v", member, target.Spec.Template.Annotations, document.MaterializationBundle.TargetPodTemplate)
				}
			}
			...
			for _, item := range plan {
				...
				case *appsv1.StatefulSet:
					for _, volume := range workload.Spec.Template.Spec.Volumes {
						...
						if volume.Secret != nil && volume.Secret.SecretName == cluster.Status.OperationWriterAccess.SecretName {
							t.Fatalf("StatefulSet %s received sealed operation-writer material %s", workload.Name, volume.Secret.SecretName)
						}
						if volume.Secret != nil && volume.Secret.SecretName == cluster.Status.CatalogAccess.SecretName {
							if workload.Name != PostgreSQLMemberStatefulSetName(cluster.Name, 0, 0) || volume.Name != catalogActivationTLSVolumeName ||
								!reflect.DeepEqual(secretItemKeys(volume.Secret.Items), []string{CatalogTLSCertificateKey, CatalogTLSPrivateKeyKey}) {
								t.Fatalf("StatefulSet %s received catalog material outside the exact source TLS projection: %#v", workload.Name, volume)
							}
						}
					}
...
func TestCatalogCandidateIdentitySealsAdmittedMaterializationUpdates(t *testing.T) {
	t.Parallel()
	cluster := testCluster()
	cluster.Status.PostgreSQLBootstraps = testPostgreSQLBootstraps(cluster)
	cluster.Status.PostgreSQLWritableLeases = testPostgreSQLWritableLeases(cluster)
	...
	if _, err := DesiredPostgreSQLCatalogCandidateConfigMaps(updated); err == nil || !strings.Contains(err.Error(), "PostgreSQL configuration creation result") {
		t.Fatalf("stale PostgreSQL configuration checkpoint was accepted after a materialization input update: %v", err)
	}
	updatedConfiguration, err := DesiredPostgreSQLConfigurationConfigMap(updated)
	if err != nil {
		t.Fatal(err)
	}
	updated.Status.PostgreSQLConfiguration = &pgshardv1alpha1.PostgreSQLConfigurationStatus{
		ConfigMapName: updatedConfiguration.Name,
		ConfigMapUID:  "updated-postgresql-configuration-uid",
		DataSHA256:    PostgreSQLConfigurationDataSHA256(updatedConfiguration),
	}
	...
	for member := range before {
		if reflect.DeepEqual(before[member].Data, after[member].Data) ||
			...
			t.Fatalf("candidate member %d did not seal the updated exact execution bundle: before=%#v after=%#v", member, before[member].Data, after[member].Data)
		}
	}
...
func TestBootstrapSourceLifecycleClassificationDoesNotRequireCatalogActivationConsumer(t *testing.T) {
	...
	legacy.Spec.Containers[0].VolumeMounts = slices.DeleteFunc(legacy.Spec.Containers[0].VolumeMounts, func(mount corev1.VolumeMount) bool {
		return mount.Name == catalogActivationJournalVolumeName || mount.Name == catalogActivationTLSVolumeName
	})
	legacy.Spec.Containers[0].Ports = slices.DeleteFunc(legacy.Spec.Containers[0].Ports, func(port corev1.ContainerPort) bool {
		return port.Name == "activation-tls"
	})
	legacy.Spec.Volumes = slices.DeleteFunc(legacy.Spec.Volumes, func(volume corev1.Volume) bool {
		return volume.Name == catalogActivationJournalVolumeName || volume.Name == catalogActivationTLSVolumeName
	})
...
func TestPostgreSQLWritableLeaseNameFitsDNSLabelAtMaximumClusterLength(t *testing.T) {
	...
	names := []string{
		...
		PostgreSQLCatalogCandidateConfigMapName(cluster, 4, strings.Repeat("f", 64)),
	}
...
func TestPostgreSQLCatalogCandidateConfigMapNameContract(t *testing.T) {
	t.Parallel()
	...
	for _, test := range []struct {
		...
		{cluster: "demo", member: 2, want: "demo-s0-m0002-cfg-" + strings.Repeat("f", 32)},
		{cluster: strings.Repeat("c", pgshardv1alpha1.MaximumClusterNameLength), member: 4, want: "ccccccccccc-5de6bf7f73e3-c0004-" + strings.Repeat("f", 32)},
	} {
		if got := PostgreSQLCatalogCandidateConfigMapName(test.cluster, test.member, digest); got != test.want ||
			!PostgreSQLCatalogCandidateConfigMapNameIsValid(test.cluster, test.member, got, digest) {
			t.Fatalf("candidate ConfigMap name = %q, want %q", got, test.want)
		}
	}
	if PostgreSQLCatalogCandidateConfigMapName("demo", 0, "invalid") != "" {
		t.Fatal("invalid candidate payload digest produced a ConfigMap name")
	}
}
...
func testPostgreSQLReplicationCredentials(cluster *pgshardv1alpha1.PgShardCluster) []pgshardv1alpha1.PostgreSQLReplicationCredentialStatus {
	...
	cluster.Status.PostgreSQLConfiguration = &pgshardv1alpha1.PostgreSQLConfigurationStatus{
		ConfigMapName: postgresqlConfiguration.Name,
		ConfigMapUID:  "test-postgresql-configuration-uid",
		DataSHA256:    PostgreSQLConfigurationDataSHA256(postgresqlConfiguration),
	}
