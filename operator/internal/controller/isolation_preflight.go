...
// dispatchProber proves per-backend webhook dispatch for the activating
// cluster's FENCED namespace via direct sentinel probes. Prove is the BASE proof
// (always-on PodCreate webhook only; runs during the INACTIVE preflight, before
// the enforcing label exists). ProveEnforcing additionally proves that EVERY
// backend dispatches each LABEL-GATED isolation webhook (workload, connect,
// LimitRange) for the now-enforcing namespace; it drives the pre-enforcement
// CONVERGE state and every in-activation revalidation. The two proofs hash into
// disjoint tuples, so one can never satisfy the other.
type dispatchProber interface {
	Prove(ctx context.Context, namespace string) (dispatchProof, error)
	ProveEnforcing(ctx context.Context, namespace string) (dispatchProof, error)
}
...
// preflightConverged composes the INACTIVE activation preflight: the build-time
// bridge ceiling AND the supported-minor gate AND base dispatch convergence.
// Every gate is fail-closed and surfaces its own typed condition. It returns the
// base dispatch proof and whether the pre-enforcement CONVERGE state may be
// entered. The controller-identity probe deliberately does NOT run here: two of
// its required observations (the Deployment and HPA controllers) are recorded
// only by the LABEL-GATED workload webhook, which cannot dispatch before the
// enforcing label is set — the probe runs in driveIsolationConverge, after the
// gated webhooks are proven dispatching, or fresh activation would deadlock.
...
// activation (QUIESCE/RECREATE/ACTIVE) with the ENFORCING proof — the receipt's
// tuple was sealed by the CONVERGE state from an enforcing proof, so every
// revalidation also re-proves that each label-gated webhook still dispatches on
// every backend (a removed enforcing label or a stale new backend fails it). It
// fails CLOSED for BOTH an invalidated tuple AND a proof read/confirmation
// ERROR: it FIRST durably drops the receipt out of ACTIVE/RECREATE to
// ACTIVATING_QUIESCE (which denies every create) and clears the seals, THEN
// returns any probe error — so a transient EndpointSlice/VWC read or
// dispatch-confirmation failure can never leave an ACTIVE receipt intact. It
// never resets to INACTIVE (fail-open). On a clean re-enumeration it re-seals
// under the fresh tuple during the quiesce pass. It returns whether the
...
func (r *PgShardClusterReconciler) revalidateDispatchTuple(ctx context.Context, cluster *pgshardv1alpha1.PgShardCluster) (bool, error) {
	...
	var proveErr error
	if r.DispatchProber != nil {
		proof, proveErr = r.DispatchProber.ProveEnforcing(ctx, cluster.Namespace)
	}
	accepted := false
	ackDetail := ""
	if proveErr == nil {
