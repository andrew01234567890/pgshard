...
const POSTGRESQL_CONF_FILE: &str = "postgresql.conf";
const POSTGRESQL_AUTO_CONF_FILE: &str = "postgresql.auto.conf";
const POSTGRESQL_CONF_NAME: &str = "PostgreSQL configuration file";
const POSTGRESQL_AUTO_CONF_NAME: &str = "PostgreSQL automatic configuration file";
const SYNCHRONOUS_CONF_NAME: &str = "PostgreSQL synchronous-replication configuration file";
const SYNCHRONOUS_CONF_STAGING_FILE: &str = ".synchronous.conf.staging";
const MAX_POSTGRESQL_CONF_BYTES: u64 = 1_048_576;
const CONFIGURATION_HEADER: &str = "# Generated by pgshard-agent. Manual edits are overwritten.\n";
/// The records every local policy opens with, and the only ones that reach a
/// database other than `postgres`.
///
/// `PostgreSQL` matches the database field with an exact comparison and treats
/// `postgres` there as a literal rather than a keyword, so the single-database
/// record these replace could never admit a session on the catalog database:
/// `local postgres postgres peer` was skipped for `shardschema` and the
/// connection fell through to the deny-all below. That refused the agent's own
/// catalog writer, which connects as `postgres` to `shardschema` over this
/// socket, under every policy the product deploys.
///
/// The peer record is widened by one database name and nothing else. It admits
/// no principal it did not already admit: peer is not a privilege boundary here
/// — the agent and the postmaster share a UID, so the account this record names
/// is already an unrestricted superuser on `postgres`, from where it can drop,
/// alter or re-own the catalog database anyway.
///
/// The two `scram-sha-256` records are what the catalog logins authenticate
/// through, and possession of the credential from the mounted Secret is the
/// discriminator that peer cannot be. They are scoped to one database and one
/// role each; without them a credential proof has no password-verifying path to
/// run against and can only ever report that none exists.
macro_rules! catalog_access_hba_records {
    () => {
        concat!(
...
/// The deny-by-default tail every local policy ends with. Every identity and
/// database the records above do not name reaches this and is refused.
macro_rules! local_deny_hba_records {
    () => {
        concat!("local all all reject\n", "local replication all reject\n")
    };
}
const QUARANTINE_HBA_CONTENT: &[u8] =
    concat!(catalog_access_hba_records!(), local_deny_hba_records!()).as_bytes();
const REPLICATION_BOOTSTRAP_PRIMARY_HBA_CONTENT: &[u8] = concat!(
    catalog_access_hba_records!(),
    local_deny_hba_records!(),
...
impl PostgresConfig {
    ...
    pub(crate) fn new_for_role(
        ...
        validate_absolute_normal_path(SYNCHRONOUS_CONF_NAME, &synchronous_conf_file, false)?;
        validate_quotable_path(SYNCHRONOUS_CONF_NAME, &synchronous_conf_file)?;
        ...
        if synchronous_conf_file.starts_with(&data_dir)
            || synchronous_conf_file.starts_with(&socket_dir)
        {
            return Err(PostgresConfigError::MutableSynchronousConfFile {
                synchronous_conf_file,
            });
        }
        // Both files are materialized read-only and then revalidated against
        // different exact bytes. Sharing one path would make each write undo
        // the other's, so the pair can never both be correct.
        if synchronous_conf_file == hba_file {
            return Err(PostgresConfigError::CollidingMaterializedFiles {
                path: synchronous_conf_file,
            });
        }
    ...
    fn synchronous_conf_contents(&self) -> Result<String, PostgresError> {
        synchronous_conf_contents(
            &self
                .generation_durability
...
struct ValidatedPostgresState {
    ...
    postgresql_conf: FileSnapshot,
    auto_conf: Option<FileSnapshot>,
    synchronous_conf_file: FileSnapshot,
...
impl PreparedPostgres {
    ...
    fn command(&self) -> Command {
        ...
        command
            ...
            // No `synchronous_standby_names` setting appears here, and none may
            // be added. It is `PGC_SIGHUP`, but an argument is `PGC_S_ARGV`,
            // which outranks the `PGC_S_FILE` a reload writes: the same
            // `set_config_with_handle` and `ProcessConfigFileInternal` rule that
            // keeps `ssl` off this command line would freeze the synchronous
            // topology for the life of the postmaster. Changing that topology is
            // exactly what the serving transition does, and it must reload
...
fn validate_prepared_state(
    ...
    let postgresql_conf = ensure_postgresql_conf_include(
        &config.data_dir,
        &config.synchronous_conf_file,
        expected_uid,
    )?;
    let auto_conf = validate_inactive_auto_conf(&config.data_dir, expected_uid)?;
    let synchronous_conf = config.synchronous_conf_contents()?;
    materialize_synchronous_conf_file(
        &config.synchronous_conf_file,
        expected_uid,
        synchronous_conf.as_bytes(),
    )?;
    let synchronous_conf_file = validate_synchronous_conf_file(
        &config.synchronous_conf_file,
        expected_uid,
        synchronous_conf.as_bytes(),
    )?;
...
/// The exact policy the agent installs for one runtime role.
///
/// Reachable across the crate so a live fixture can install the policy the
/// product installs rather than a copy of it. A fixture holding its own copy is
/// a fixture that can be more permissive than anything deployed, which is how a
/// policy that refuses the agent's own catalog writer reads as green.
pub(crate) const fn hba_policy_contents(role: PostgresRuntimeRole) -> &'static [u8] {
    match role {
        PostgresRuntimeRole::Quarantine
        | PostgresRuntimeRole::ReplicationStandby
...
fn materialize_hba_file(
    ...
    ensure_materialized_directory(parent, expected_uid, name)?;
    ...
    match fs::remove_file(&staging_path) {
        ...
        Err(source) if source.kind() == std::io::ErrorKind::NotFound => {}
        Err(source) => {
            return Err(PostgresError::Materialize {
                name,
                operation: "remove interrupted staging file",
                path: staging_path,
                source,
    ...
    drop(staging_file);
    fs::rename(&staging_path, path).map_err(|source| PostgresError::Materialize {
        name,
...
fn ensure_materialized_directory(
    path: &Path,
    expected_uid: u32,
    name: &'static str,
) -> Result<(), PostgresError> {
...
/// Renders the one setting the agent reloads, refusing any value that could not
/// survive being written as a single-quoted `PostgreSQL` value. Every accepted
/// topology is a list of canonical member names, so a value carrying a quote or
/// a newline is a corrupted caller rather than a configuration to escape.
fn synchronous_conf_contents(setting: &str) -> Result<String, PostgresError> {
    if !setting
        .bytes()
        .all(|byte| matches!(byte, b'A'..=b'Z' | b'a'..=b'z' | b'0'..=b'9' | b'_' | b' ' | b'(' | b')' | b','))
    {
        return Err(PostgresError::UnquotableSynchronousStandbyNames {
            value: setting.to_owned(),
        });
    }
    Ok(format!(
        "{CONFIGURATION_HEADER}synchronous_standby_names = '{setting}'\n"
    ))
}

/// Writes the reloadable setting the agent is about to be judged against, then
/// leaves it read-only for the same reason [`materialize_hba_file`] does: the
/// validator refuses anything the runtime identity can still rewrite, and agent
/// and postmaster share a UID. Unlike the HBA policy the intended bytes change
/// with the configured topology, so being idempotent means matching the value
/// this call is about to install, not a constant.
fn materialize_synchronous_conf_file(
    path: &Path,
    expected_uid: u32,
    contents: &[u8],
) -> Result<(), PostgresError> {
    // Preparation validates, then spawn validates again and compares snapshots.
    // Replacing an already-correct file would change its inode between those two
    // reads and be indistinguishable from tampering, so leave it alone.
    if validate_synchronous_conf_file(path, expected_uid, contents).is_ok() {
        return Ok(());
    }
    ...
    ensure_materialized_directory(parent, expected_uid, SYNCHRONOUS_CONF_NAME)?;

    let staging_path = parent.join(SYNCHRONOUS_CONF_STAGING_FILE);
    match fs::remove_file(&staging_path) {
        Ok(()) => {}
        Err(source) if source.kind() == std::io::ErrorKind::NotFound => {}
        Err(source) => {
            return Err(PostgresError::Materialize {
                name: SYNCHRONOUS_CONF_NAME,
                operation: "remove interrupted staging file",
                path: staging_path,
                source,
            });
    ...
    drop(staging_file);
    // A reload can arrive at any moment, so the installed file is replaced whole
    // rather than rewritten in place: the postmaster must never parse a partial
    // setting.
    fs::rename(&staging_path, path).map_err(|source| PostgresError::Materialize {
        name: SYNCHRONOUS_CONF_NAME,
        operation: "install staged configuration",
        path: path.to_owned(),
        source,
    })
}

fn validate_synchronous_conf_file(
    path: &Path,
    expected_uid: u32,
    expected_contents: &[u8],
) -> Result<FileSnapshot, PostgresError> {
    let metadata = strict_metadata(SYNCHRONOUS_CONF_NAME, path)?;
    require_regular(SYNCHRONOUS_CONF_NAME, path, &metadata)?;
    if metadata.uid() != 0 && metadata.uid() != expected_uid {
        return Err(PostgresError::WrongOwner {
            name: SYNCHRONOUS_CONF_NAME,
            path: path.to_owned(),
    ...
    if metadata.len() != expected_contents.len() as u64 {
        return Err(PostgresError::InvalidSynchronousConf {
            path: path.to_owned(),
        });
    }
    let contents = fs::read(path).map_err(|source| PostgresError::Read {
    ...
    Ok(file_snapshot(&strict_metadata(
        SYNCHRONOUS_CONF_NAME,
        path,
    )?))
}

fn postgresql_conf_include(synchronous_conf_file: &Path) -> Vec<u8> {
    let mut directive = Vec::from(b"\ninclude_if_exists '".as_slice());
    directive.extend_from_slice(synchronous_conf_file.as_os_str().as_bytes());
    directive.extend_from_slice(b"'\n");
    directive
}
...
///
/// The directive goes last because `PostgreSQL` takes the final assignment among
/// equal-ranked file sources, so anything the deployment wrote above it loses.
/// It is `include_if_exists` because bootstrap and quarantine paths can start a
/// postmaster before any agent has materialized the target, and a missing file
...
fn ensure_postgresql_conf_include(
    ...
    let path = data_dir.join(POSTGRESQL_CONF_FILE);
    let metadata = strict_metadata(POSTGRESQL_CONF_NAME, &path)?;
    require_regular(POSTGRESQL_CONF_NAME, &path, &metadata)?;
    if metadata.uid() != expected_uid {
        return Err(PostgresError::WrongOwner {
    ...
    if metadata.len() > MAX_POSTGRESQL_CONF_BYTES {
        return Err(PostgresError::OversizedConfiguration {
            name: POSTGRESQL_CONF_NAME,
            path,
            bytes: metadata.len(),
            maximum: MAX_POSTGRESQL_CONF_BYTES,
        });
    }
    let contents = fs::read(&path).map_err(|source| PostgresError::Read {
        name: POSTGRESQL_CONF_NAME,
    ...
    let directive = postgresql_conf_include(synchronous_conf_file);
    if !contents.ends_with(&directive) {
        let fd = open(
            &path,
        ...
        .map_err(|source| PostgresError::Materialize {
            name: POSTGRESQL_CONF_NAME,
            operation: "open for the agent include",
            path: path.clone(),
            source: source.into(),
        })?;
        let mut file = File::from(fd);
        file.set_len(settled_conf_len(&contents, &directive) as u64)
            ...
            .map_err(|source| PostgresError::Materialize {
                name: POSTGRESQL_CONF_NAME,
                operation: "append the agent include",
                path: path.clone(),
                source,
            })?;
    }
    Ok(file_snapshot(&strict_metadata(
        POSTGRESQL_CONF_NAME,
        &path,
    )?))
}

/// Refuses a `postgresql.auto.conf` that carries settings.
///
...
fn validate_inactive_auto_conf(
    ...
    let path = data_dir.join(POSTGRESQL_AUTO_CONF_FILE);
    match fs::symlink_metadata(&path) {
        Ok(_) => {}
        Err(source) if source.kind() == std::io::ErrorKind::NotFound => return Ok(None),
        Err(source) => {
            return Err(PostgresError::Metadata {
                name: POSTGRESQL_AUTO_CONF_NAME,
                path,
                source,
            });
        }
    }
    let metadata = strict_metadata(POSTGRESQL_AUTO_CONF_NAME, &path)?;
    require_regular(POSTGRESQL_AUTO_CONF_NAME, &path, &metadata)?;
    if metadata.uid() != expected_uid {
        return Err(PostgresError::WrongOwner {
            name: POSTGRESQL_AUTO_CONF_NAME,
            path,
    ...
    if contents.split(|byte| *byte == b'\n').any(|line| {
        ...
        return Err(PostgresError::ActiveAutomaticSettings { path });
    }
    Ok(Some(file_snapshot(&strict_metadata(
        POSTGRESQL_AUTO_CONF_NAME,
        &path,
    )?)))
}
...
/// Refuses a path the agent could not name inside a single-quoted `PostgreSQL`
/// configuration value. The include directive this feeds is appended to durable
/// PGDATA state, so a path carrying a quote or a newline would leave a
/// deployment mistake behind as a permanent directive of someone else's shape.
fn validate_quotable_path(name: &'static str, path: &Path) -> Result<(), PostgresConfigError> {
    let bytes = path.as_os_str().as_bytes();
    if bytes
        .iter()
    ...
    {
        return Err(PostgresConfigError::UnquotablePath {
            name,
            path: path.to_owned(),
        });
    }
...
pub enum PostgresConfigError {
    ...
    /// The reloadable settings file was placed where `PostgreSQL` itself can rewrite it.
    #[error(
        "PostgreSQL synchronous-replication configuration file {synchronous_conf_file:?} must not be stored inside PGDATA or the socket directory"
    )]
    MutableSynchronousConfFile {
        /// Rejected path.
        synchronous_conf_file: PathBuf,
    },
    /// Two agent-materialized files were pointed at one path.
    #[error("PostgreSQL agent-materialized files must not share the path {path:?}")]
    CollidingMaterializedFiles {
        /// Shared path.
        path: PathBuf,
    },
    /// A path cannot be written as one single-quoted `PostgreSQL` configuration value.
    #[error(
        "{name} path {path:?} must be printable ASCII without quotes, backslashes, or control characters"
    )]
    UnquotablePath {
        /// Configuration field.
        name: &'static str,
...
pub enum PostgresError {
    ...
    /// An agent-owned `PostgreSQL` input file could not be written.
    #[error("materialize {name} {path:?}: {operation}: {source}")]
    Materialize {
        /// Which file was being written.
        name: &'static str,
        /// Step that failed.
        operation: &'static str,
        /// Path being written.
    ...
    /// The materialized synchronous-replication file did not hold the exact bytes.
    #[error(
        "PostgreSQL synchronous-replication configuration {path:?} does not hold the exact agent-owned setting"
    )]
    InvalidSynchronousConf {
        /// Rejected path.
        path: PathBuf,
    },
    /// A rendered setting could not be represented as one quoted `PostgreSQL` value.
    #[error("PostgreSQL synchronous_standby_names value {value:?} contains unquotable bytes")]
    UnquotableSynchronousStandbyNames {
        /// Rejected value.
        value: String,
    },
    /// `postgresql.auto.conf` could outrank the settings the agent materializes.
    #[error(
        "PostgreSQL automatic configuration {path:?} holds active settings, which are applied after every agent-owned file"
    )]
    ActiveAutomaticSettings {
        /// Rejected path.
        path: PathBuf,
    },
    /// A bounded configuration file was larger than the agent will read.
    #[error("{name} {path:?} is {bytes} bytes, above the {maximum} byte bound")]
...
mod tests {
    ...
    fn config_rejects_overlap_unsafe_socket_and_excessive_budget() {
        let config = |socket_dir: PathBuf, smart, fast, immediate| {
            PostgresConfig::new(
                PathBuf::from("/data"),
                PathBuf::from("/bin/postgres"),
                socket_dir,
        ...
        let smart = Duration::from_secs(5);
        let fast = Duration::from_secs(40);
        let immediate = Duration::from_secs(5);
        assert!(matches!(
            config(PathBuf::from("/data/socket"), smart, fast, immediate),
            Err(PostgresConfigError::OverlappingPaths { .. })
        ));
        assert!(matches!(
            config(PathBuf::from("/run/bad,socket"), smart, fast, immediate),
            Err(PostgresConfigError::UnsafeSocketPath(_))
        ));
        assert!(matches!(
            config(PathBuf::from("/run/bad "), smart, fast, immediate),
            Err(PostgresConfigError::UnsafeSocketPath(_))
        ));
        assert!(
            config(
    ...
    /// The two policies as the image bakes them, at the paths the runtime reads
    /// them from.
    ///
    /// These are not decoration. The bootstrap-source agent is handed
    /// `/etc/pgshard/replication-bootstrap-primary.pg_hba.conf` as its own
    /// `PGSHARD_POSTGRES_HBA_FILE`, and the file is baked root-owned and `0444`
    /// into a root-owned directory. [`validate_hba_file`] compares it against
    /// [`hba_policy_contents`] and [`materialize_hba_file`] cannot repair what
    /// it cannot write, so a shipped file that disagrees with the constant is a
    /// postmaster that never starts. The operator's standby bootstrap reads both
    /// files directly as well.
    ///
    /// Included rather than opened. A test that read a path could pass because
    /// the path was missing, and the compiled test binary is shipped in the
    /// image and run where the repository does not exist — so a run-time read
    /// would be both weaker and unrunnable there.
    ///
    /// This lives under `cfg(test)` so the shipped binaries do not embed bytes
    /// they never read: the agent loads its policy from disk. The image compiles
    /// this crate's test target anyway, for the fence test binary, so the build
    /// context carries these files either way.
    const BAKED_QUARANTINE_HBA: &[u8] =
        include_bytes!("../../../deploy/images/quarantine.pg_hba.conf");
    const BAKED_REPLICATION_BOOTSTRAP_PRIMARY_HBA: &[u8] =
        include_bytes!("../../../deploy/images/replication-bootstrap-primary.pg_hba.conf");

    const fn bytes_are_identical(left: &[u8], right: &[u8]) -> bool {
    ...
    /// Divergence fails the build of anything that compiles this test target —
    /// `cargo clippy --all-targets`, `cargo test --no-run`, and the image stage
    /// that produces the fence test binary — rather than waiting for a test run.
    const _: () = assert!(
        bytes_are_identical(QUARANTINE_HBA_CONTENT, BAKED_QUARANTINE_HBA),
        "deploy/images/quarantine.pg_hba.conf is not the policy the agent installs for the \
         quarantine role; the image would bake a policy the agent refuses"
    );
    const _: () = assert!(
        bytes_are_identical(
            REPLICATION_BOOTSTRAP_PRIMARY_HBA_CONTENT,
            BAKED_REPLICATION_BOOTSTRAP_PRIMARY_HBA
        ),
        "deploy/images/replication-bootstrap-primary.pg_hba.conf is not the policy the agent \
         installs for the replication-bootstrap-primary role; the image would bake a policy the \
         agent refuses and the postmaster would never start"
    );

    /// The compile-time assertions above already refuse a divergence, so
    /// reaching this test means one got through. It exists for what it prints: a
    /// build failure names the file, this names the bytes.
    #[test]
    fn the_baked_policies_are_the_policies_the_agent_installs() {
        for (baked, installed, path) in [
            (
                BAKED_QUARANTINE_HBA,
                hba_policy_contents(PostgresRuntimeRole::Quarantine),
                "deploy/images/quarantine.pg_hba.conf",
            ),
            (
                BAKED_REPLICATION_BOOTSTRAP_PRIMARY_HBA,
                hba_policy_contents(PostgresRuntimeRole::ReplicationBootstrapPrimary),
                "deploy/images/replication-bootstrap-primary.pg_hba.conf",
            ),
        ] {
            assert_eq!(
                std::str::from_utf8(baked).expect("the baked policy is text"),
                std::str::from_utf8(installed).expect("the installed policy is text"),
                "{path} is baked into the image at the path the agent reads as its own \
                 PGSHARD_POSTGRES_HBA_FILE, and the agent cannot repair a root-owned file. \
                 A policy it disagrees with is a postmaster that never starts."
            );
        }
    }

    /// Every policy the agent installs has to admit the agent's own catalog
    /// writer, which connects as `postgres` to `shardschema` over the local
    /// socket.
    ///
    /// `PostgreSQL` compares the database field with an exact match, so a
    /// record naming only `postgres` there is skipped for `shardschema` and the
    /// connection falls through to the deny-all. That refused catalog
    /// materialization under every deployed policy, and no test observed it
    /// because the live fixture ran a stock `trust` policy instead.
    #[test]
    fn every_policy_admits_the_catalog_writer_to_the_catalog_database() {
        for role in [
            PostgresRuntimeRole::Quarantine,
            PostgresRuntimeRole::ReplicationStandby,
            PostgresRuntimeRole::ServingPrimary,
            ...
            let policy = std::str::from_utf8(hba_policy_contents(role)).expect("policy is text");
            let admitting = policy
                .lines()
                .take_while(|line| *line != "local all all reject")
                .find(|line| {
                    let mut fields = line.split_whitespace();
                    fields.next() == Some("local")
                        && fields.next().is_some_and(|databases| {
                            databases.split(',').any(|database| database == "shardschema")
                        })
                        && fields.next() == Some("postgres")
                })
                .unwrap_or_else(|| {
                    panic!("{role:?} has no record admitting postgres to shardschema before its deny-all")
                });
            assert!(
                admitting.ends_with(" peer"),
                "{role:?} admits the catalog writer by something other than peer: {admitting}"
            );
        }
    }

    /// The deny-by-default tail is the posture every record above it is carved
    /// out of. Widening it, or letting a record above it name `all` databases or
    /// `all` roles, hands the whole cluster to whatever reaches the socket.
    #[test]
    fn no_policy_widens_the_local_deny_by_default_tail() {
        for role in [
            PostgresRuntimeRole::Quarantine,
            PostgresRuntimeRole::ReplicationStandby,
            PostgresRuntimeRole::ServingPrimary,
            ...
            let policy = std::str::from_utf8(hba_policy_contents(role)).expect("policy is text");
            assert!(
                policy.contains("local all all reject\nlocal replication all reject\n"),
                "{role:?} does not deny every local identity it did not name"
            );
            for line in policy
                .lines()
                .take_while(|line| *line != "local all all reject")
            {
                ...
                let [connection, databases, roles, method, ..] = fields.as_slice() else {
                    panic!("{role:?} carries a record with too few fields: {line}");
                };
                assert_eq!(
                    *connection, "local",
                    "{role:?} places a non-local record above its local deny-all: {line}"
                );
                assert!(
                    !databases.split(',').any(|database| database == "all"),
                    "{role:?} admits every database before its deny-all: {line}"
                );
                assert!(
                    !roles.split(',').any(|role_name| role_name == "all"),
                    "{role:?} admits every role before its deny-all: {line}"
                );
                assert!(
                    matches!(*method, "peer" | "scram-sha-256"),
                    "{role:?} admits a local identity by {method}, which does not \
                     require either the operating-system account or the credential: {line}"
                );
            }
        }
    }

    /// The catalog logins are not superusers and do not own the runtime UID, so
    /// the only thing that can distinguish them is the credential the mounted
    /// Secret carries. `peer` cannot: the agent and the postmaster share a UID,
    /// so it would admit anything already on the socket.
    #[test]
    fn the_catalog_logins_authenticate_by_the_credential_and_not_by_the_uid() {
        for role in [
            PostgresRuntimeRole::Quarantine,
            PostgresRuntimeRole::ReplicationStandby,
            PostgresRuntimeRole::ServingPrimary,
            ...
            let policy = std::str::from_utf8(hba_policy_contents(role)).expect("policy is text");
            for login in ["pgshard_pooler_catalog", "pgshard_orchestrator_catalog"] {
                let record = policy
                    .lines()
                    .take_while(|line| *line != "local all all reject")
                    .find(|line| line.split_whitespace().nth(2) == Some(login))
                    .unwrap_or_else(|| panic!("{role:?} carries no local record for {login}"));
                assert_eq!(
                    record,
                    &format!("local shardschema {login} scram-sha-256"),
                    "{role:?} admits {login} on something other than the catalog database \
                     through something other than the credential"
                );
            }
        }
    }
...
host all all all reject\n";
    ...
    /// validation a new inode it cannot tell from tampering.
    #[test]
    fn materializes_the_synchronous_topology_and_replaces_it_only_when_the_value_changes() {
        let fixture = TempDir::new().expect("create configuration fixture");
        let path = fixture.path().join("conf").join("synchronous.conf");
        let uid = geteuid().as_raw();
        let local = synchronous_conf_contents("").expect("render local topology");

        materialize_synchronous_conf_file(&path, uid, local.as_bytes())
            .expect("materialize into a missing directory");
        assert_eq!(
            fs::read_to_string(&path).expect("read materialized configuration"),
            "# Generated by pgshard-agent. Manual edits are overwritten.\n\
             synchronous_standby_names = ''\n"
        );
        assert_eq!(
            fs::metadata(&path)
                .expect("stat configuration")
                .permissions()
                .mode()
                & 0o7_777,
            0o400,
            "a file the runtime identity can rewrite is one the validator rejects"
        );
        validate_synchronous_conf_file(&path, uid, local.as_bytes())
            .expect("the validator must accept what we wrote");

        let installed = fs::metadata(&path).expect("stat configuration").ino();
        materialize_synchronous_conf_file(&path, uid, local.as_bytes())
            .expect("materialize the same value again");
        assert_eq!(
            fs::metadata(&path).expect("stat configuration").ino(),
            installed,
            "re-materializing an unchanged value must not replace the file -- a new inode \
             is indistinguishable from tampering when spawn re-reads the snapshot"
        );

        let remote = synchronous_conf_contents("ANY 1 (pgshard_member_0001, pgshard_member_0002)")
            .expect("render remote topology");
        materialize_synchronous_conf_file(&path, uid, remote.as_bytes())
            .expect("install a new topology");
        assert_eq!(
            fs::read_to_string(&path).expect("read new configuration"),
            "# Generated by pgshard-agent. Manual edits are overwritten.\n\
        ...
        assert_ne!(
            fs::metadata(&path).expect("stat configuration").ino(),
            installed,
            "a changed value must arrive by replacement so a reload never reads a partial \
             setting"
        );
        validate_synchronous_conf_file(&path, uid, remote.as_bytes())
            .expect("the new topology must validate");
    }

    #[test]
    fn rejects_a_synchronous_configuration_that_was_rewritten_or_left_writable() {
        let fixture = TempDir::new().expect("create configuration fixture");
        let path = fixture.path().join("synchronous.conf");
        let uid = geteuid().as_raw();
        let expected = synchronous_conf_contents("").expect("render local topology");

        fs::write(&path, "synchronous_standby_names = 'ANY 1 (attacker)'\n")
            .expect("write foreign configuration");
        fs::set_permissions(&path, fs::Permissions::from_mode(0o400)).expect("seal configuration");
        assert!(matches!(
            validate_synchronous_conf_file(&path, uid, expected.as_bytes()),
        ...
        materialize_synchronous_conf_file(&path, uid, expected.as_bytes())
            .expect("repair a rewritable configuration");
        validate_synchronous_conf_file(&path, uid, expected.as_bytes())
            .expect("the repaired configuration must validate");
    }

    /// `PostgreSQL` takes the last assignment among equal-ranked file sources, so
    /// the agent's include only decides the setting while it stays last.
    #[test]
    fn the_agent_include_is_appended_once_and_kept_last() {
        let fixture = pgdata_fixture();
        let conf = PathBuf::from("/run/pgshard/conf/synchronous.conf");
        let uid = geteuid().as_raw();
        let path = fixture.path().join(POSTGRESQL_CONF_FILE);

        let first = ensure_postgresql_conf_include(fixture.path(), &conf, uid)
            .expect("append the agent include");
        let contents = fs::read_to_string(&path).expect("read configuration");
        assert!(
            contents.ends_with("\ninclude_if_exists '/run/pgshard/conf/synchronous.conf'\n"),
            "the agent include must be the last directive: {contents:?}"
        );
        assert_eq!(
            contents.matches("include_if_exists").count(),
            1,
            "a settled include must not be appended again: {contents:?}"
        );

        let second = ensure_postgresql_conf_include(fixture.path(), &conf, uid)
            .expect("re-run against a settled configuration");
        assert_eq!(
            first, second,
            "re-running must not disturb the snapshot spawn compares against"
        );
        assert_eq!(
        ...
        tampered
            .write_all(b"synchronous_standby_names = 'ANY 1 (attacker)'\n")
            .expect("append a later assignment");
        drop(tampered);
        ensure_postgresql_conf_include(fixture.path(), &conf, uid)
            .expect("restore the agent include");
        let restored = fs::read_to_string(&path).expect("read configuration");
        assert!(
            restored.ends_with("\ninclude_if_exists '/run/pgshard/conf/synchronous.conf'\n"),
            "an assignment written after the include must be outranked again: {restored:?}"
        );
    }
    ...
    /// agent materializes.
    #[test]
    fn refuses_active_settings_in_the_automatic_configuration() {
        let fixture = pgdata_fixture();
        let uid = geteuid().as_raw();
        let path = fixture.path().join(POSTGRESQL_AUTO_CONF_FILE);

        assert_eq!(
            validate_inactive_auto_conf(fixture.path(), uid).expect("absent is inert"),
            None
        );

        fs::write(&path, b"# Do not edit this file manually!\n").expect("write header only");
        fs::set_permissions(&path, fs::Permissions::from_mode(0o600)).expect("protect file");
        assert!(
            validate_inactive_auto_conf(fixture.path(), uid)
                .expect("a comment-only file overrides nothing")
                .is_some()
        );

        fs::write(
            &path,
            b"# Do not edit this file manually!\n  synchronous_standby_names = 'ANY 1 (attacker)'\n",
        )
        .expect("write an active setting");
        assert!(matches!(
            validate_inactive_auto_conf(fixture.path(), uid),
    ...
    #[test]
    fn refuses_a_synchronous_topology_that_cannot_be_quoted() {
        for hostile in [
            "ANY 1 (a)'\nssl = on\n#",
            "ANY 1 (a\\b)",
            "ANY 1 (a)\"",
            ...
            assert!(
                matches!(
                    synchronous_conf_contents(hostile),
                    Err(PostgresError::UnquotableSynchronousStandbyNames { .. })
                ),
                "{hostile:?} must not be written into a configuration file"
            );
        }
    }

    #[test]
    fn rejects_synchronous_configuration_paths_that_cannot_be_owned_or_quoted() {
        let durations = || {
            (
                Duration::from_secs(5),
                Duration::from_secs(40),
        ...
        for rejected in [
            PathBuf::from("/data/synchronous.conf"),
            PathBuf::from("/run/postgres/synchronous.conf"),
        ] {
            ...
            assert!(
                matches!(
                    ...
                    Err(PostgresConfigError::MutableSynchronousConfFile { .. })
                ),
                "{rejected:?} is rewritable by the postmaster the agent supervises"
            );
        }
        ...
        assert!(matches!(
            ...
            Err(PostgresConfigError::UnquotablePath { .. })
        ));
        ...
        assert!(matches!(
            ...
            Err(PostgresConfigError::CollidingMaterializedFiles { .. })
        ));
    }

    #[test]
    fn preparation_materializes_the_configured_topology_and_spawn_agrees_with_it() {
        let fixture = pgdata_fixture();
        ...
        let synchronous_conf = runtime.path().join("conf").join("synchronous.conf");
        ...
        let prepared = prepare_fixture(config.clone()).expect("prepare bootstrap primary");
        assert_eq!(
            fs::read_to_string(&synchronous_conf).expect("read materialized topology"),
            "# Generated by pgshard-agent. Manual edits are overwritten.\n\
             synchronous_standby_names = 'ANY 1 (pgshard_member_0001, pgshard_member_0002)'\n",
            "the topology that left the command line must arrive in the included file"
        );
        assert!(
            fs::read_to_string(fixture.path().join(POSTGRESQL_CONF_FILE))
                .expect("read configuration")
                .ends_with(&format!(
                    "\ninclude_if_exists '{}'\n",
                    synchronous_conf.display()
                )),
            "an unincluded file cannot carry the setting to the postmaster"
        );
    ...
    fn materialize_refuses_when_the_policy_directory_cannot_be_written() {
        ...
        assert!(
            matches!(result, Err(PostgresError::Materialize { .. })),
    ...
    /// `synchronous_standby_names` is `PGC_SIGHUP` under the same rule: naming it
    ...
    fn no_role_pins_the_synchronous_topology_on_the_postmaster_command_line() {
        ...
        fs::set_permissions(&primary_hba, fs::Permissions::from_mode(0o400))
            .expect("protect replication HBA");
        let primary = PostgresConfig::new_for_role(
            PostgresRuntimeRole::ReplicationBootstrapPrimary,
            None,
            GenerationDurability::remote_apply_any_one(vec![
                "pgshard_member_0001".to_owned(),
        ...
        for (role, config) in [
            ...
            let pinned: Vec<_> = arguments
                ...
                .filter(|argument| {
                    argument
                        .as_bytes()
                        .starts_with(b"synchronous_standby_names")
                })
                .collect();
            assert!(
                pinned.is_empty(),
    ...
    async fn requested_shutdown_signals_and_reaps_the_direct_child() {
        ...
        let config = PostgresConfig::new(
            ...
            root.path().join("socket"),
            hba_file.clone(),
            synchronous_conf_beside(&hba_file),
            Duration::from_millis(500),
            Duration::from_millis(500),
            Duration::from_millis(500),
        )
    ...
    fn populate_pgdata(path: &Path) {
        ...
        fs::write(
            path.join(POSTGRESQL_CONF_FILE),
        ...
        .expect("protect configuration file");
        fs::write(
    ...
    fn synchronous_conf_beside(hba_file: &Path) -> PathBuf {
        hba_file.with_file_name("synchronous.conf")
    }
