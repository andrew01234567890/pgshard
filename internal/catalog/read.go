...
// The values of pgshard.databases.ddl_transactions. Sequential lets a
// transaction holding only DDL run it statement by statement.
const (
	DDLTransactionsAtomic     = "atomic"
	DDLTransactionsSequential = "sequential"
)
...
type TableStatus struct {
	...
	Migrating bool
	// ReferenceCheckedGeneration is the effective generation the controller
	// last inspected this table's shards for. Nil means never, which a
	// router reads as unsafe rather than clean.
	ReferenceCheckedGeneration *int64
