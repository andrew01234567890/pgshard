...
func TestPlacementIdentityColumnsOnPostgres(t *testing.T) {
	...
	for id := range int32(2) {
		...
		// GENERATED ALWAYS tag is not given, GENERATED BY DEFAULT seq is
		// auto-assigned; both must exceed the copied maxima on this shard.
		var seq, tag int64
		if err := c.QueryRow(context.Background(), `INSERT INTO things (tenant, note) VALUES ($1, 'fresh') RETURNING seq, tag`, tenant).Scan(&seq, &tag); err != nil {
