...
func TestClassifyPodDistinguishesReloadFromRestart(t *testing.T) {
	...
	tpl2 := Template(c2, Group{}, nil, nil)
	...
	if s := classifyPod(pod, Template(c3, Group{}, nil, nil), false); !s.restart {
		t.Fatalf("image change must restart: %+v", s)
	}
	c4 := c.DeepCopy()
	c4.Annotations = map[string]string{AnnotationRestart: "now"}
	if s := classifyPod(pod, Template(c4, Group{}, nil, nil), false); !s.restart {
	...
	if s := classifyPod(pod, Template(c5, Group{}, nil, nil), false); !s.restart {
...
func TestTemplateHashesSplitPodShapeFromSettings(t *testing.T) {
	c := newCluster("th")
	first := Template(c, Group{}, nil, nil).Hash()
	if Template(c.DeepCopy(), Group{}, nil, nil).Hash() != first {
		t.Fatal("hash must be deterministic")
	}
	tuned := pgtune.Settings{{Name: "shared_buffers", Value: "1GB"}}
	if Template(c, Group{}, nil, nil).Hash() != Template(c, Group{}, tuned, nil).Hash() {
		t.Fatal("settings must not move the pod hash; their own hash tracks them")
	}
	if Template(c, Group{}, tuned, nil).SettingsHash() == Template(c, Group{}, nil, nil).SettingsHash() {
	...
	if got := Template(c, Group{}, tuned, nil).Settings["shared_buffers"]; got != "1GB" {
...
func TestConfigMapCarriesOverrideAndSettings(t *testing.T) {
	...
	if !contains(cm.Data[agentConfigKey(g.MemberName(1))], `"settingsHash": "`+Template(c, Group{}, tuning, nil).SettingsHash()+`"`) {
