...
func TestSighupSettingChangeReloadsWithoutRestart(t *testing.T) {
	...
	want := Template(c, nil).SettingsHash()
...
func TestPostmasterSettingChangeRollsStandbysThenSwitchesOver(t *testing.T) {
	...
	if got := settingsStamp(t, "rr-shard-0-1"); got != Template(c, nil).SettingsHash() {
	...
	if got := settingsStamp(t, oldPrimary); got != Template(c, nil).SettingsHash() {
