...
func golden() []want {
	return []want{
		...
		{sql: "copy regions from stdin", kind: Refuse, msg: "COPY on a reference table is not available yet"},
		...
		{sql: "select * from orders o left join order_lines l on o.tenant_id = l.tenant_id and l.tenant_id = 7", kind: Scatter, shards: "all"},
		{sql: "select * from orders o right join order_lines l on o.tenant_id = l.tenant_id and o.tenant_id = 7", kind: Scatter, shards: "all"},
		{sql: "select * from orders o full join order_lines l on o.tenant_id = l.tenant_id and l.tenant_id in (7, 8)", kind: Scatter, shards: "all"},
		...
		{sql: "select * from orders union all select * from order_lines", kind: Scatter, shards: "all"},
		{sql: "select * from orders where tenant_id = 1 union all select * from orders", kind: Scatter, shards: "all"},
		{sql: "select * from orders union select * from order_lines", kind: Refuse, msg: "multi-shard SELECT with set operations"},
		...
		{sql: "select * from orders o join order_lines l on o.tenant_id = l.tenant_id", kind: Scatter, shards: "all"},
		...
		{sql: "copy orders from stdin", kind: Refuse, msg: "COPY into a sharded table must name its columns"},
		{sql: "copy orders to stdout", kind: Refuse, msg: "COPY TO from a sharded table is not available yet"},
