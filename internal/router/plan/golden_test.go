...
func golden() []want {
	return []want{
		...
		{sql: "set local search_path to app", kind: SessionLocal},
		...
		{sql: "explain select * from orders", kind: Scatter, shards: "all"},
		{sql: "select count(*) from orders", kind: Refuse, msg: "scatter SELECT with aggregates is not available yet"},
		{sql: "select max(id) from orders", kind: Refuse, msg: "scatter SELECT with aggregates is not available yet"},
		{sql: "select count(distinct id) from orders", kind: Refuse, msg: "scatter SELECT with aggregates is not available yet"},
		{sql: "select id from orders group by id", kind: Refuse, msg: "scatter SELECT with GROUP BY/HAVING is not available yet"},
		{sql: "select distinct id from orders", kind: Refuse, msg: "scatter SELECT with DISTINCT is not available yet"},
		{sql: "select * from orders order by id", kind: Refuse, msg: "scatter SELECT with ORDER BY is not available yet"},
		{sql: "select * from orders limit 10", kind: Refuse, msg: "scatter SELECT with LIMIT/OFFSET is not available yet"},
		{sql: "select * from orders offset 10", kind: Refuse, msg: "scatter SELECT with LIMIT/OFFSET is not available yet"},
		{sql: "select * from orders order by id limit 10", kind: Refuse, msg: "scatter SELECT with ORDER BY, LIMIT/OFFSET is not available yet"},
		{sql: "select row_number() over () from orders", kind: Refuse, msg: "scatter SELECT with aggregates is not available yet"},
		{sql: "select * from orders for update", kind: Refuse, msg: "scatter SELECT with FOR UPDATE/SHARE is not available yet"},
		...
		{sql: "with c as (select * from orders) select * from c", kind: Refuse, msg: "scatter SELECT with common table expressions is not available yet"},
		{sql: "select (select count(*) from order_lines) from orders", kind: Refuse, msg: "cross-shard join"},
		{sql: "select * from (select * from orders) s", kind: Refuse, msg: "scatter SELECT with subqueries is not available yet"},
		{sql: "select * from orders where id in (select order_id from order_lines)", kind: Refuse, msg: "cross-shard join"},
		{sql: "select * from orders o cross join generate_series(1, 3) g", kind: Refuse, msg: "scatter SELECT with joins, function scans is not available yet"},
