...
func TestDDLClassification(t *testing.T) {
	cases := []struct {
		...
		{sql: "create unlogged sequence s2", refuse: "CREATE UNLOGGED SEQUENCE is not supported"},
		{sql: "create table t4 (id int)", mig: "CREATE TABLE home", object: "relation:t4:present"},
		...
		{sql: "alter table orders add column _pgroll_new_tenant_id int8", refuse: "a pgroll migration of the shard key column \"tenant_id\" of sharded table \"orders\" is not available"},
		{sql: "alter table orders add column if not exists _pgroll_new_tenant_id int8 not null default 0", refuse: "a pgroll migration of the shard key column"},
		{sql: "alter table orders add column _pgroll_new_note text", mig: "ALTER TABLE all"},
		{sql: "alter table regions add column _pgroll_new_name text", refuse: "a pgroll migration of reference table \"regions\" is not available"},
		{sql: "alter table t1 add column _pgroll_new_id int", mig: "ALTER TABLE home"},
		...
		{sql: "alter table items set unlogged", refuse: "ALTER TABLE ... SET UNLOGGED is not supported"},
		{sql: "alter table items set logged", refuse: "rewrite-class DDL is not available yet: SET LOGGED"},
