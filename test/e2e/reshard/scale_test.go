...
func waitForLedgerRole(ctx context.Context, t *testing.T, c *e2e.Cluster) {
	...
	waitForWhy(ctx, t, c, "ledger role applied on every group", 3*time.Minute, func() string {
		return "\nrole state: " + catalogSQL(ctx, t, c,
			...
			"\ndesired: " + catalogSQL(ctx, t, c, "SELECT "+rolesDesiredGeneration)
	}, func() bool {
		return catalogSQL(ctx, t, c, `SELECT count(*) = 0 FROM pgshard.role_group_status
			WHERE roles_generation < `+rolesDesiredGeneration) == "t" &&
			catalogSQL(ctx, t, c, "SELECT count(*) > 0 FROM pgshard.role_group_status") == "t"
	})
}

// rolesDesiredGeneration is the desired roles generation, and must stay the
// same expression as (*catalog.RoleStore).Desired computes: one sequence
// stamps all four desired-state tables, so the max over ONE of them is a
// generation the controller passed some time ago, not the one it is working
// towards. A new desired-state table has to be added here too.
