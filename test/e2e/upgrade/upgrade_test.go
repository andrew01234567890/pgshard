...
func TestUpgrade18To19UnderLoad(t *testing.T) {
	...
	waitFor(ctx, t, "first acknowledged ledger writes", 2*time.Minute, func() bool { return l.acked.Load() >= 25 })

	if err := c.Apply(ctx, clusterManifest(19, "2m")); err != nil {
		t.Fatal(err)
	}
	record := clusterName + "-reshard-g2"
	waitFor(ctx, t, "upgrade record in mode upgrade", 5*time.Minute, func() bool {
		return jsonpath(ctx, c, "pgshardreshard", record, "{.spec.mode}") == "upgrade"
	})
	waitFor(ctx, t, "kind=upgrade workflow running", 5*time.Minute, func() bool {
	...
	waitFor(ctx, t, "writes switched to the 19 set", 30*time.Minute, func() bool {
	...
	waitFor(ctx, t, "writes flowing on the new major", 3*time.Minute, func() bool { return l.acked.Load() > 0 && servingShardGroup(ctx, c) == "shard-0-g2" })
	...
	waitFor(ctx, t, "rollback to the 18 set", 15*time.Minute, func() bool {
		return servingShardGroup(ctx, c) == "shard-0"
	})
	waitFor(ctx, t, "rolled-back workflow closed", 10*time.Minute, func() bool {
		st := upgradeWorkflowState(ctx, t, c, "g2")
		return strings.HasPrefix(st, "cancelled:") || strings.HasPrefix(st, "completed:") || st == ""
	})
	waitFor(ctx, t, "writes flowing again on 18", 5*time.Minute, func() bool { return l.acked.Load() > ackedAtSwitch })
	l.verify(ctx, t, l.acked.Load())

	// The spec still asks for 19, so a fresh run (g3) starts and this time
	// runs to retirement.
	record3 := clusterName + "-reshard-g3"
	waitFor(ctx, t, "re-run upgrade record", 10*time.Minute, func() bool {
		return jsonpath(ctx, c, "pgshardreshard", record3, "{.spec.mode}") == "upgrade"
	})
	waitFor(ctx, t, "writes switched to the 19 set again", 30*time.Minute, func() bool {
	...
	waitFor(ctx, t, "old groups retired and deleted", 20*time.Minute, func() bool {
	...
	waitFor(ctx, t, "catalog group on 19", 25*time.Minute, func() bool {
	...
	waitFor(ctx, t, "catalog upgrade finished", 15*time.Minute, func() bool {
...
func TestUpgrade18To19ChaosControllerAndPrimaryKill(t *testing.T) {
	...
	waitFor(ctx, t, "upgrade workflow copying", 15*time.Minute, func() bool {
	...
	waitFor(ctx, t, "upgrade completed after the chaos", 30*time.Minute, func() bool {
		return strings.HasPrefix(upgradeWorkflowState(ctx, t, c, "g2"), "completed:") &&
			jsonpath(ctx, c, "pgshardcluster", clusterName, "{.status.servingPGMajor}") == "19"
	})
	waitFor(ctx, t, "writes flowing after the chaos", 10*time.Minute, func() bool {
		before := l.acked.Load()
