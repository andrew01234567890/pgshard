...
| Scatter UPDATE/DELETE, `INSERT ... SELECT`, multi-row INSERT spanning shards, COPY TO from sharded tables | Planned | refused with `0A000` |
...
| Session state replay over transaction pooling; cancel; COPY (unsharded, and FROM STDIN into sharded tables with two-phase commit); drain on SIGTERM; peer cancel forwarding | Implemented | `internal/router`, `cancelpeer` |
...
| pgroll against a sharded database: init, baseline, start, complete, rollback | Partial | `add_column` and `create_index` reach every shard, proved by the real binary in `test/e2e/router/pgrollsharded_test.go`; the other five metadata operations the design expects to work are not yet exercised (PGS-929); a migration needing pgroll's dual-write backfill is not supported on a non-local database, because its backfill is a single scalar cursor. The shard key and reference tables are refused at start. See [guide/pgroll.md](guide/pgroll.md) |
