...
| `streams` | Change streams: `name`, `state` (`creating`, `active`, `lost`), `database`, `two_phase`, `created_at`; `pgshard_admin` may insert, update and delete. The `spec` and `position` columns migration `0009` replaced were dropped in `0031` |
| `stream_status` | Since `0009`: per `(stream, shard_set, shard_id)` the slot name, `wal_status` (`lost` once invalidated, `missing` when absent), `invalidation_reason`, `confirmed_flush_lsn`, `restart_lsn`, `active`, `updated_at`, and since `0010` `retained_bytes` (WAL behind `restart_lsn`), `synced`, `failover`; written by the controller's stream monitor |
...
| `restore_points` | `id` (uuid), `name` (unique), `shard_map_generation`, `per_group` (jsonb: `{group: {lsn, timeline, wal_segment, system_identifier}}`; `system_identifier` lets a restore tell a catalog rebuilt by a major upgrade since), `certified` (since `0006`: writes were paused and two-phase commits drained when the points were taken), `created_at` |
