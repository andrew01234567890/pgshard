...
- **Query duration.** `--max-query-duration` bounds how long one statement
  may run before the router cancels it as if the client had asked; the
...
| `GROUP BY` including the shard key, `DISTINCT` (or `DISTINCT ON`) including the shard key | unchanged | every group or distinct row lives on one shard, so the streams are concatenated (or merged for `ORDER BY`); aggregates in such a query are computed on the shards. The reference has to be **the sharded relation's** key: a reference table is copied to every shard, so a same-named column on one (`regions.tenant_id` joined to `orders`) has rows for one value on every shard and is refused. An unqualified name is accepted only when every relation in play is sharded — the router does not know a reference table's columns, and PostgreSQL does not say which relation it resolved the name to |
...
`pgshard.functions` (see [catalog.md](catalog.md)) lifts the refusal for that
database, and so does grouping on the shard key, which makes every group
shard-local,
