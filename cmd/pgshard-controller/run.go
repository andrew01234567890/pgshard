...
func runController(ctx context.Context, args []string, stdout, stderr io.Writer) int {
	...
	ownedRangeEvery := fs.Duration("owned-range-interval", 30*time.Second, "time between passes recording each shard's own keyspace range in every user database")
	ownerGuardEvery := fs.Duration("owner-guard-interval", 10*time.Second, "time between passes installing, on every shard, the check that refuses a sharded table's row whose key belongs to another shard")
	...
	*catalogDSN = withPassword
	...
	if *shardDSNTemplate != "" || len(shardDSNs) > 0 {
		...
		// shards paused. This lifts both as soon as no barrier can still be
		...
		go (&controller.OwnedRanges{Pool: pool, Shards: dialer, Logger: logger}).Run(ctx, *ownedRangeEvery, leader)
		go (&controller.OwnerGuard{Pool: pool, Shards: dialer, Logger: logger}).Run(ctx, *ownerGuardEvery, leader)
