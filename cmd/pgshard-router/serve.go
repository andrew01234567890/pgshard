...
func runServe(ctx context.Context, args []string, stdout, stderr io.Writer) int {
	...
	maxQueryDuration := fs.Duration("max-query-duration", 0, "time one statement may run before the router cancels it (0 leaves it unbounded, as PostgreSQL's own statement_timeout does); the cancel reaches the shard and the stream is torn down after its grace")
