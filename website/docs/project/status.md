...
| `shardschema` catalog and cache | PostgreSQL 18 idempotent migration, dual-CAS route activation, commit-only notification, live PG18 tests, owned idle LISTEN-before-load reader, bounded notification coalescing and 1-to-300-second periodic refresh driver, fresh-session reconnect with jittered bounded backoff, fail-closed initial readiness, bounded stale-cache grace, metrics-ready supervision state, bounded repeatable-read snapshots, validated checksums and lock-free retained-epoch cache; pooler composition, TLS, timeouts, and HTTP/Prometheus publication are absent | Partial |
