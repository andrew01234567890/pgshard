...
| Pooling and SQL routing | Bounded zero-copy PostgreSQL 18 frontend frame plus selected simple/extended query-body decoders, byte/token/AST/stack-bounded permissive candidate parsing with a live PostgreSQL 18 positive/known-negative smoke corpus, fail-closed bound-parameter routing core with exact canonical hashing, and standalone microbenchmarks; no listening endpoint, TLS/authentication, semantic route planner, or connection pool | Partial |
