...
| Pooling and SQL routing | Bounded zero-copy PostgreSQL 18 frontend frame plus selected simple/extended query-body decoders, byte/token/AST/stack-bounded permissive candidate parsing with a live PostgreSQL 18 positive/known-negative smoke corpus, a catalog-bound template for one deliberately narrow parameterized `SELECT` shape, exact PostgreSQL parameter-type resolution gated by all-active-shard physical type/collation/encoding/schema-epoch proof, live coercible-column, empty-path, and cached-plan re-analysis boundary tests, fail-closed bound-parameter routing with exact canonical hashing, and standalone microbenchmarks; no listening endpoint, TLS/authentication, authoritative schema-observation runtime, session enforcement, Bind integration, complete semantic planner, or connection pool | Partial |
