...
production mutation path today. Direct placement transitions are rejected;
switching requires a future atomic target-fenced cutover API and controller.
Reshard lifecycle controllers remain unimplemented. Durable DDL, backup/restore and
...
bootstrap-sensitive session defaults. After locking every existing catalog
relation, it rejects external inheritance and requires constraints, defaults,
generated expressions, predicates, and indexes to match a SHA-256 fingerprint
from a supported release before reading rows or issuing seed DML. The Rust loader
also reads catalog relations with `ONLY`. A transaction cannot by itself lock
